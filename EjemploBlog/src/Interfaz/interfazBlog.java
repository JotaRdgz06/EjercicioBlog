...
public class interfazBlog {
	...
	private static int publicacionATrabajar = 0;
	...
	public static void trabajarConBlog() throws Exception{
		...
		System.out.print("\nIngrese el código del blog con el que desea trabajar: ");
        blogATrabajar = scanner.nextInt();
        
        if (!blogs.containsKey(blogATrabajar)) {
            throw new Exception("no existe un blog con ese codigo");
        }
        
        int opcion = 0;
        while (opcion != 3) {
        	System.out.println(menuBlog());
    ...
    public static void verPubliYComent() {
    	try {
	    	...
	    	publicacionATrabajar = scanner.nextInt();
	...
	public static void crearComentario() throws Exception {
		scanner.nextLine();
		...
		if (ip.isEmpty())
			throw new Exception("ip no puede estar vacio");
		if (texto.isEmpty())
			throw new Exception("texto no puede estar vacio");
		
		controladora.agregarComentario(blogATrabajar, publicacionATrabajar, email, ip, texto);
	}
	
	public static void borrarComentario() throws Exception {
        System.out.print("Ingrese la posición del comentario que desea borrar ");
        int posicion = scanner.nextInt();

        controladora.borrarComentario(blogATrabajar, publicacionATrabajar, posicion);
    }
	...
	public static void borrarBlog() throws Exception{
        ...
        if (blogs.isEmpty())
            throw new Exception("no hay blogs para borrar");
        System.out.println("Elija el numero de blog que desea borrar:");
        for (Integer id : blogs.keySet()) {
	        String nombre = blogs.get(id);
	        System.out.println("\nID: " + id + "\nBlog: " + nombre);
	    }
        ...
        if (!blogs.containsKey(codigo))
            throw new Exception("No existe un blog con ese código.");
        try {
	...
	public static void porDefecto() {
		...
		controladora.agregarComentario(1, 1, "unRandom@gmail.com", "231.577.1.7", "estimado usuario de esta red social, con el debido respeto me refiero a usted para expresar mi desacuerdo con su opinion, en primer lugar el nodo centinela es completamente \n"
				+ "indetectable ya que se ignora por completo al no poseer informacion en él, haciendo que ni siquiera el \n"
				+ "ultra instinto pueda detectarlo, en segunda, si bien pueden estar en una escala de poder muy similar, el poder del nodo centinela se \n"
				+ "asimila más al de saitama, siendo que entre más fuerte es su rival, más crecerá la diferencia de poder entre ambos, siendo algo \n"
				+ "exponencial que no tiene limites conocidos, esto se demuestra en la pagina 25 del quinto tomo del manga del \n"
				+ "nodo centinela, aunque es entendible que no cuente con esta informacion ya que no ha sido animado. Es por esto y por otras razones \n"
				+ "que considero que su afirmacion es erronea, buenas tardes.");
		...
		controladora.crearPublicacion(2, "yo creo que el mio es Haru Urara del umamusume", "su personalidad tan feliz y positiva, su espiritu competitivo y la historia de la yegua de la vida real me hacen admirarla, porque a pesar de correr en 113 carreras \n"
				+ "y no ganar ninguna nunca se rindió y se esforzó por mejorar, por eso la admiro y me motiva a seguir adelante", "MrYeiYei");
		controladora.agregarComentario(2, 4, "XxCarlitosProYTxX", "367.368.1.8", "yo tambien admiro a rice shower y a TM Opera O, porque fueron tan buenos caballos que la gente los odiaba por ganar las carrearas, pero a pesar de eso siempre se convirtieron \n"
				+ "en leyendas, me gusta mucho sus historias");
